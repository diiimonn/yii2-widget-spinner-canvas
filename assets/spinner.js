 ...
 * Math
 */
window['pyth'] = function(a,b) { return Math.sqrt(a*a + b*b); };
window['degrees'] = function(radian) { return (radian*180) / Math.PI; };
window['radian'] = function(degrees) { return (degrees*Math.PI) / 180; };
...
var All = {
  ...
  remove: function(element) {
    $($.map(this.spinners, function(spinner) {
      return (_.isElement(element) ? spinner.element == element :
        $(spinner.element).is(element)) ? spinner.element : null;
...
$.extend(Spinner.prototype, {
  ...
  pause: function() {
    if (this._state == 'paused') return null;
  ...
  stop: function() {
    if (this._state == 'stopped') return null;
